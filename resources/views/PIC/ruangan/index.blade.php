...
<div class="card">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @endphp
                        <span class="badge {{ $sCls }}" style="white-space:nowrap;">{{ $sLabel }}</span>
                    </td>
                    <td>
                        <div style="display:flex;gap:6px;">
                            <a href="{{ route('pic.ruangan.edit', $ruangan->id) }}"
                               class="btn btn-outline btn-sm">Edit</a>
                            <button type="button" class="btn btn-danger btn-sm"
                                onclick="bukaModalRuanganPic('{{ $ruangan->id }}', '{{ addslashes($ruangan->nama_ruangan) }}')">
                                Hapus
                            </button>
                        </div>
                    </td>
    ...
    <div class="pagination-wrap">
        {{ $ruangans->links('layouts.pagination') }}
    </div>
    @endif
</div>

{{-- Modal Konfirmasi Hapus Ruangan --}}
<div id="modalHapusRuanganPic" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);
    ...
    <div style="background:#fff;border-radius:12px;padding:32px 28px;width:100%;max-width:400px;
        ...
        <div style="text-align:center;margin-bottom:24px;">
            ...
            <div style="font-size:14px;color:var(--text-muted);line-height:1.5;">
                Anda akan menghapus ruangan <strong id="modalNamaRuanganPic"></strong>.
                Tindakan ini <strong>permanen</strong> dan tidak dapat dibatalkan.
            </div>
        </div>

        <div style="display:flex;gap:10px;">
            <button type="button" onclick="tutupModalRuanganPic()"
            ...
            <button type="button" onclick="document.getElementById('formHapusRuanganPic').submit()"
...
<form id="formHapusRuanganPic" method="POST" style="display:none;">
    @csrf
    @method('DELETE')
</form>

<script>
const baseUrlRuanganPic = "{{ url('pic/ruangan') }}";

function bukaModalRuanganPic(id, nama_ruangan) {
    document.getElementById('modalNamaRuanganPic').textContent = nama_ruangan;
    document.getElementById('formHapusRuanganPic').action = `${baseUrlRuanganPic}/${id}`;
    document.getElementById('modalHapusRuanganPic').style.display = 'flex';
}

function tutupModalRuanganPic() {
    document.getElementById('modalHapusRuanganPic').style.display = 'none';
}

document.getElementById('modalHapusRuanganPic').addEventListener('click', function(e) {
    if (e.target === this) tutupModalRuanganPic();
});
</script>
