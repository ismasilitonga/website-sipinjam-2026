@extends(auth()->user()->role == 'admin' ? 'layouts.admin' : 'layouts.pic')
...
<div class="card" style="max-width:680px;">
    <div class="card-header">
        <span class="card-title">
            {{ isset($ruangan) ? 'Form Edit Ruangan' : 'Form Ruangan Baru' }}
        </span>
    </div>
    ...
    <div class="card-body">
        ...
        @if(auth()->user()->role == 'admin')
            ...
            @if(isset($ruangan))
                <form method="POST"
                      action="{{ route('admin.ruangan.update', $ruangan->id) }}"
        ...
        @else
            ...
            @if(isset($ruangan))
                <form method="POST"
                      action="{{ route('pic.ruangan.update', $ruangan->id) }}"
                      enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
            @else
                <form method="POST"
                      action="{{ route('pic.ruangan.store') }}"
                      enctype="multipart/form-data">
                    @csrf
            @endif
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-select" required>
                    ...
                    <option value="tidak_tersedia"
                        {{ old('status', $ruangan->status ?? '') == 'tidak_tersedia' ? 'selected' : '' }}>
                        Tidak Tersedia
                    </option>
                </select>
