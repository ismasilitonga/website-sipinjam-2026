...
<div class="card">
    ...
    <div style="display:flex;gap:10px;">
        <a href="{{ route('admin.laporan-insiden.pdf') }}" class="btn btn-primary btn-sm">
            📄 PDF
        </a>
        <a href="{{ route('admin.laporan-insiden.excel') }}" class="btn btn-sm"
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div style="font-weight:600;font-size:13px;">{{ $ins->user->nama ?? '-' }}</div>
                        <div style="font-size:11px;color:var(--text-muted);">{{ $ins->user->organisasi ?? '' }}</div>
                    </td>
                    <td>
                        <div style="font-weight:500;font-size:13px;max-width:180px;">
                            {{ $ins->judul }}
                        </div>
                        @if($ins->foto)
                    ...
                    <td style="font-size:12px;color:var(--text-muted);max-width:160px;">
                        @if($ins->tindak_lanjut)
                            ...
                            <div style="font-size:11px;margin-top:2px;">
                                {{ \Carbon\Carbon::parse($ins->ditindak_at)->format('d M, H:i') }}
                            </div>
                            @endif
                        @else
...
<script>
    function openTindakModal(id, judul, tindakLanjut, status) {
        document.getElementById('tindakForm').action = '/admin/laporan-insiden/' + id;
