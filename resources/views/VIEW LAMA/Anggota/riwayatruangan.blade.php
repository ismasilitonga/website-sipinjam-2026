...
<head>
    ...
    <title>Riwayat Peminjaman Ruangan - SC-Space</title>
    <style>
        /* --- CSS DASAR --- */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #clock-display { color: white; font-weight: 500; font-size: 15px; padding: 5px 12px; font-family: monospace; background: none !important; }
        ...
        .notif-header { padding: 12px 15px; background: #f8fbff; border-bottom: 1px solid #eee; font-weight: 600; color: #2f7ea1; }

        .profile { position: relative; cursor: pointer; }
        .profile-circle { width: 40px; height: 40px; background: #ffffff; border-radius: 50%; display: flex; justify-content: center; align-items: center; }
        ...
        .dropdown {
            ...
            opacity: 0; transform: translateY(-10px); pointer-events: none; transition: 0.25s; z-index: 1001; padding: 8px 0;
        }
        ...
        .dashboard-menu { padding: 12px 18px; font-weight: 600; display: flex; align-items: center; gap: 12px; text-decoration: none; color: black; font-size: 16px; }
        ...
        .menu-title span { display: flex; align-items: center; gap: 12px; }
        .submenu { display: none; padding-left: 25px; }
        .submenu a { display: block; padding: 6px 14px; text-decoration: none; color: black; margin: 2px 0; border-radius: 6px; font-size: 16px; }
        ...
        .content-wrapper {
            ...
            padding: 40px 20px; display: flex; justify-content: center; position: relative; z-index: 10;
        }
        ...
        .status-container { display: flex; align-items: center; justify-content: center; gap: 8px; font-weight: 600; }
        .dot { width: 10px; height: 10px; border-radius: 50%; display: inline-block; }
        .dot-pending { background-color: #ffc107; }
        .dot-approved { background-color: #28a745; }
        .dot-rejected { background-color: #dc3545; }

        .btn-action-detail { background: linear-gradient(140deg, #2f7ea1, #4a9dc3); color: white; padding: 8px 16px; border-radius: 10px; font-size: 12px; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }
    ...
    <div class="nav-right">
        ...
        <div class="notif-container">
            ...
            <div class="notif-dropdown" id="notifDropdown">
                ...
                <div class="notif-body"><div style="padding: 30px; text-align: center; color: #94a3b8; font-style: italic;">Tidak ada notifikasi baru.</div></div>
            </div>
        </div>
...
<div class="main">
    <div class="sidebar">
        <div class="sidebar-content">
            <a href="javascript:void(0)" class="dashboard-menu" onclick="goToPage('dashboard')"><i class="fa-solid fa-gauge"></i> <span>Dashboard</span></a>
            ...
            <div class="menu-group">
                ...
                <div class="submenu">
                    <a href="javascript:void(0)" onclick="goToPage('buatlaporan')">Buat Laporan</a>
            ...
            <div class="menu-group">
                ...
                <div class="submenu" style="display: block;">
                    <a href="javascript:void(0)" class="active" onclick="goToPage('peminjamanruangan')">Peminjaman Ruangan</a>
                    <a href="javascript:void(0)" onclick="goToPage('peminjamanbarang')">Peminjaman Barang</a>
                </div>
            </div>
        </div>
        ...
        <div class="sidebar-footer">
            <div class="profile-info" onclick="goToPage('profile')">
                <div class="profile-avatar">{{ strtoupper(substr(Auth::user()->name ?? 'I', 0, 1)) }}</div>
                <div class="profile-details">
                    <h4>{{ Auth::user()->name ?? 'Isma Anggota' }}</h4>
                    <p>{{ Auth::user()->nim ?? '3312301022' }}</p>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="status-container">
                                    <span class="dot {{ $data->status == 'pending' ? 'dot-pending' : ($data->status == 'disetujui' ? 'dot-approved' : 'dot-rejected') }}"></span>
                                    {{ ucfirst($data->status) }}
                                </div>
                            </td>
                            <td>
                                <a href="/Anggota/detailruangan/{{ $data->id }}" class="btn-action-detail">Detail <i class="fa-solid fa-chevron-right"></i></a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="5" style="padding: 80px 0; color: #94a3b8; font-style: italic;">
                                <i class="fa-solid fa-folder-open" style="display: block; font-size: 30px; margin-bottom: 10px; opacity: 0.5;"></i>
...
<script>
    ...
    function goToPage(page) {
        const routes = { 'dashboard': '/Anggota/dashboardanggota', 'profile': '/Anggota/detailakun', 'peminjamanruangan': '/Anggota/riwayatruangan' };
